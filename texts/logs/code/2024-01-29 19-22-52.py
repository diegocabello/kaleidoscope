    ...
    if (0 < sentence.find('”') + 1 < len(sentence)) or (sentence[1:].find('“') >= 1):
        ...
        for index, value in enumerate(lister):
            if index == 0 and not sentence.startswith('“'):
                speaker = re.search(r'(Mr\.|Mrs\.|Ms\.|Miss)\s', value) if re.search(r'(Mr\.|Mrs\.|Ms\.|Miss)\s', value) else '' + str(entitier(inputer = value)[0]) if entitier(inputer = value) else None
