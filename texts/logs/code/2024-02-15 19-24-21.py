...
def parse_book(book_name=None):
    ...
    chapter_list = ['CHAPTER ' + chapter for chapter in text.split('CHAPTER') if chapter][0:15]      #chapter_list = [chapter[(chapter.find('. \r\n')+1):] for chapter in chapter_list]
    ...
    for chapter_counter, chapter in enumerate(chapter_list):                                                    #    ====CHAPTER====
        ...
        workbook.conditional_formatting.add(f'F3:N{str(start_index)}', true_rule)
        workbook.conditional_formatting.add(f'F3:F{str(start_index)}', false_rule)
