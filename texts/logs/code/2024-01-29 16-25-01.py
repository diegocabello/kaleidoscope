    ...
    if (0 < sentence.find('”') + 1 < len(sentence)) or (sentence[1:].find('“') >= 1):
        ...
        for index, value in enumerate(lister):
            if index == 0 and not sentence.startswith('“'):
                speaker = str(entitier(inputer = value)[0]) if entitier(inputer = value) else ''
    ...
    for count, chapter in enumerate(chapter_list):                                                              #    ====CHAPTER====
        ...
        for section in chapter:                                                                                 #    ====SECTION====
            ...
            if typer == 'quote':
                if prev_type == 'quote':
                    speaker1 = speaker
                    speaker2 = get_speaker(section) if get_speaker(section) else ''  # pushes it back and redefines it if there
                else:
                    speaker = get_speaker(section) if get_speaker(section) else ''   # defines it if not there

                if prev_type == 'quote': 
                    speaker_list = [speaker2, speaker1]
                    speaker = speaker_list.reverse()[1]
            else:
                speaker = None
            ...
            for counter, sentence in enumerate(sentence_list):
                if not sentence or not sentence.isspace():
                    ...
                    if typer == 'quote':
                        frame = 'speaker' if counter == 0 else 'spoken about'
                    else: 
                        frame = None
                    ...
                    for counterrr, cell in enumerate(sheet[start_index]):
                        cell.fill, cell.border = columns_values[columns[counterrr][0]]['color'], border # sorry that is a mess 
