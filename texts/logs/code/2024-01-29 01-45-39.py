...
def section_parser(sentence):
    ...
    if '“' in sentence:   #     if sentence.startswith('“'): # this is for the context `Alice said, “something.”`
        if type == 'paragraph': # this is neccassary if it ends with a long dash as i cant access the regex 
...
def main(book_name=None):
    ...
    for count, chapter in enumerate(chapter_list):
        chapter_list[count] = [section + '\r\n\r\n' for section in chapter.split('\r\n\r\n')] # these are not regular quote marks, they are fancy unicode quotes; chapter.split('”\r\n\r\n“')
    ...
    for count, chapter in enumerate(chapter_list):
        ...
        for section in chapter: 
            ...
            sentence_list = []
            for sentence in chapter:                                    # splits the sentences into sentences
                sentence_list.extend(sentencer(inputer = sentence))     
