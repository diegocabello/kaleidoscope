...
true_rule = FormulaRule(formula=[true_formula], fill=solid_fill('2a4225'))
false_rule = FormulaRule(formula=[false_formula], fill=solid_fill('a2a2a2'))
...
def parse_book(book_name=None):
    ...
    workbook_path = f'texts\\{book_name}.xlsm'
    ...
    chapter_list = ['CHAPTER ' + chapter for chapter in text.split('CHAPTER') if chapter][0:25]      #chapter_list = [chapter[(chapter.find('. \r\n')+1):] for chapter in chapter_list]
    ...
    if os.path.exists(workbook_path):
        subprocess.run(f'del "texts\\{book_name}.xlsm"', shell=True)
    ...
    for chapter_counter, chapter in enumerate(chapter_list):                                                    #    ====CHAPTER====
        ...
        sheet.conditional_formatting.add(f'F3:F{str(start_index - 1)}', false_rule)
    ...
    workbook.save(f'texts\\logs\\excel\\{book_name} {run_log_time}.xlsm')
