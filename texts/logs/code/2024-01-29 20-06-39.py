    ...
    if (0 < sentence.find('”') + 1 < len(sentence)) or (sentence[1:].find('“') >= 1):
        ...
        for index, value in enumerate(lister):
            if index == 0 and not sentence.startswith('“'):
                speaker = re.compile(r'(Mr\.|Mrs\.|Ms\.|Miss)').search(value).group(1) if re.compile(r'(Mr\.|Mrs\.|Ms\.|Miss)').search(value).group(1) else '' + str(entitier(inputer = value)[0]) if entitier(inputer = value) else None
