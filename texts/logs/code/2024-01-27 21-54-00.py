...
def main(book_name=None):
    ...
    for count, chapter in enumerate(chapter_list):
        ...
        sentence_list = [sub_sentence + ';' if len(sentence.split(';')) > 1 and sub_sentence != sentence.split(';')[-1] else sub_sentence for sentence in sentence_list for sub_sentence in sentence.split(';') if sub_sentence and len(sentence) > 50] 
        ...
        for counter, sentence in enumerate(sentence_list):
            if not sentence or not sentence.isspace():
                ...
                for cell in sheet[counter + 3]:
                    cell.fill, cell.border = solid_fill(color if (cell.column != 'D' and len(sentence) > cut_by_value) else 'c00000'), border
