        ...
        for section_counter, section in enumerate(chapter):                                                     #    ====SECTION====
            ...
            for sentence_counter, sentence in enumerate(to_pass):                                         #    ====EXCEL====
                if not sentence or not sentence.isspace():
                    ...
                    columns_values = {
                        'section': {'value': None, color: solid_fill(section_color)},
                        'bit': {'value': None, 'color': solid_fill(bit_color), 'alignment': None},
                        'sentence': {'value': sentence + ' ', 'color': solid_fill('c00000') if len(sentence) < 40 else color, 'alignment': Alignment(vertical='center', wrap_text=True)},
                        'type': {'value': typer, 'color': color, 'alignment': None},
                        'character length': {'value': len(sentence), 'color': color, 'alignment': None},
